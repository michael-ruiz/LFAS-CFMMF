...
def get_save_path(config, localtime):
    ...
    if config.is_Multi:
        ...
        # Include guidance_modality or adaptive in folder name
        guidance = getattr(config, 'guidance_modality', 'depth')
        adaptive = getattr(config, 'adaptive_guidance', False)
        suffix = 'adaptive' if adaptive else guidance
        config.flod_name = config.model + '_Multi' + '_' + str(config.image_size) + '_' + suffix
        ...
        file_name = config.model + '_' + 'fusion' + '_' + str(config.image_size) + '_' + suffix + '_' + localtime + '_' + '.txt'
